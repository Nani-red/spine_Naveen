...
| [perl-support-roadmap](perl-support-roadmap.md) | **P1–P5 landed with evidence** (10th front-end: every `package`/5.38 `class` as `Type`, inheritance across five spellings, `CALLS` across six shapes incl. D10's verified-only default-export/`@ISA` resolution, Mojolicious/Lite/Dancer2 routes as `Endpoint`+`EXPOSES`, typed-receiver `CALLS`, DBIx::Class `Entity`/`Field`/`REFERENCES` via `__PACKAGE__->table(...)`, P5's `roadmap-status.py`/`validate-frontend.py`/language-track template); full-repo validation on both validation repos still pending | P6 review; branch `feat/perl-support` |
