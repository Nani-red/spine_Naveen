...
| [perl-support-roadmap](perl-support-roadmap.md) | **P1–P3 done** (10th front-end: every `package`/5.38 `class` as `Type`, inheritance across five spellings, `CALLS` across six shapes incl. D10's verified-only default-export/`@ISA` resolution, Mojolicious/Lite/Dancer2 routes as `Endpoint`+`EXPOSES`, typed-receiver `CALLS`) | P4 DBIx::Class, P5 generic work, P6 review; branch `feat/perl-support` |
