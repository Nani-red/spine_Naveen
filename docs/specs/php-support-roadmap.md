...
| `new X(...)` | X by the namespace rule (`use` map → current namespace) | `CALLS` → the `Type` node (corpus rule: instantiation is a call to the type). A same-namespace guess that turns out wrong is repointed by `finalize`, exactly like an `IMPLEMENTS` guess (same bare-`Type`-id shape) |
...
|---|---|---|
| Laravel | `Route::get('/orders', [OrderController::class, 'index'])` in `routes/*.php`; `Route::prefix('/v1')->group(fn () => …)`; string `'Ctl@index'` form | `Endpoint` + `EXPOSES` → `php:NS.Ctl.index`. Literal paths only; a closure handler yields the endpoint and no `EXPOSES` (the Gin rule). `Route::any`/`match` → **nothing** (D2 of [endpoints-typescript-go.md](endpoints-typescript-go.md): no `ANY`). `Route::resource` expands to seven verbs — **defer**, measure demand first |
| Symfony | `#[Route('/orders', methods: ['GET'])]` on a method, with a class-level prefix | `Endpoint` + `EXPOSES` — the ASP.NET attribute-controller shape from `csharp_extractor.py`; a `#[Route]` with no `methods:` is verb-less → nothing (same D2) |
